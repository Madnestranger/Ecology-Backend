...
router.get('/', function (req, res, next) {
    db.get().collection('pollutionsRecreationsWater').find().toArray(function (err, docs) {
        if (err) {
            console.log(err);
...
router.post('/', function (req, res) {
    var artist = {
        ...
        recreationZoneName: req.body.recreationZoneName
    };
    db.get().collection('pollutionsRecreationsWater').insert(artist, function (err, result) {
        if (err) {
            console.log(err);
            return res.sendStatus(500);
        } else {
